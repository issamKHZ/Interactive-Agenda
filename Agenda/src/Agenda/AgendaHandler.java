...
public class AgendaHandler {
	...
	public static void checkLastTask(Agenda agenda) {
		...
		while (index < agenda.getTasks().size() && agenda.getTasks().get(index).getEndTime().isBefore(now)) {
			index++;
		}
		
		if (index == 0) {
			task = agenda.getTasks().get(agenda.getTasks().size()-1);
		} else {
			task = agenda.getTasks().get(index-1);
		}
